...
const canSwitchType = computed(() =>
  OPTION_TYPES.includes(props.question.type),
);
      ...
      <template v-if="hasOptions">
        ...
        >
          ...
          >
            <span
              class="option-mark"
              :class="question.type.startsWith('checkbox') ? 'is-square' : ''"
            />
            ...
            >
              <template #suffix>
                <span v-if="opt.score != null" class="option-score-suffix"
                  >分</span
                >
              </template>
            </el-input-number>
        ...
        <button
          type="button"
          class="btn-text-primary add-option"
          ...
          <div v-else-if="question.type === 'list'" class="list-editor">
            ...
            <div class="list-head">
              <p class="list-hint">
                ...
                <span v-if="listCols.length >= 20" class="list-warn"
                  >已达 20 列上限</span
                >
              </p>
              ...
              <button
                type="button"
                class="btn-text-primary list-settings-btn"
            ...
            >
              <div class="table-preview__head">
                <div
                  ...
                  "
                >
                  <span class="table-preview__cell-text">{{
                    col.name || "未命名列"
                  }}</span>
              ...
              <div class="table-preview__body">
                ...
                >
                  <div
                    v-for="col in listCols"
                    :key="col.id"
                    class="table-preview__cell table-preview__cell--body ff-list-cell"
                    :class="{ 'is-fixed': col.width != null }"
                    :style="
                      col.width != null ? { width: col.width + 'px' } : null
                    "
            ...
            <!-- 「+ 添加一行」：应用端填报时才生效；编辑器内仅作预览展示（与 FormFill 一致） -->
            <button
              type="button"
...
.question-card {
  ...
  /* 「+ 添加一行」预览：复用全局 .btn-dashed-full,编辑器内不可交互 */
  .ff-list-add-row {
    cursor: default;
  ...
  /* 预览图：与 FormFill 列表同结构 —— 表头 + 2 行 × 真实 EP 控件（disabled） */
  .list-table-preview {
    cursor: pointer;
    transition: border-color var(--dur) var(--ease);
    ...
    /* 覆盖 global：单行 flex body → 多行纵向 stack */
    .table-preview__body {
      flex-direction: column;
    }

    /* 每行 body：横向 flex，底部 1px 分隔，最后一行无 */
    .table-preview__row {
      display: flex;
      align-items: stretch;
      min-width: 0;
    ...
    /* 单元格内放 EP 控件：padding 清零让控件撑满 cell */
    .ff-list-cell {
      padding: 0;
    }
    ...
    /* 单元格内 EP 控件：去边框 / 撑满 cell，与 FormFill 列表保持一致 */
