          ...
          >
            ...
            <div v-for="q in card.questions" :key="q.id" class="preview-question">
              ...
              <template v-else>
                ...
                <div v-else-if="q.type === 'image'" class="upload-box">+ 上传图片</div>
                ...
                <div v-else-if="q.type === 'list'" class="pq-list-wrap">
                  ...
                  <button type="button" class="btn-text-primary-sm pq-list-add-row">+ 添加一行</button>
                </div>
...
.preview-dialog {
  ...
  .preview-header {
    ...
    width: 100%;
    /* 原 .preview-header .el-radio-button__inner (gap/font-size) 已迁到 element-overrides.less */
  }
  ...
  .preview-card {
    ...
    /* 业务样式：1px 分隔线 — PreviewDialog 独有，不并到 global */
    .preview-question {
      padding: var(--sp-md) 0;
  ...
  .pq-option {
    ...
    /* 默认项视觉：.option-mark 由 global 提供，此处仅覆写默认态样式 */
    &.is-default .option-mark {
    ...
    &.is-default span:last-child {
      color: var(--c-primary);
    }
  }

  /* option-mark 主体由 global 提供；预览场景需 ::after 圆点 → 改为 relative 让 ::after 绝对定位 */
  .option-mark {
    position: relative;
  }
  ...
  .pq-list-wrap {
    ...
    /* 列表加行按钮：走全局 .btn-text-primary-sm,此处仅控制对齐 */
    .pq-list-add-row {
      align-self: flex-start;
    }
  }

  /* ---------- 单元格内 input / select 占位 ---------- */
  .pq-cell-input {
    width: 100%;
    min-width: 80px;
    height: 26px;
